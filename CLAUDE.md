...
- **BIP-322** signatures for Bitcoin address authentication (see `lib/bip322.ts`,
  `lib/signing-message.ts`, `lib/request-auth.ts`)
...
## Governance — moved out of this codebase

The DIESEL governance surface was removed from alkanes.build. There is no
governance UI, API route, component or data dependency here any more, and every
former entry point permanently redirects to the canonical venue at
`https://surtur.org/proposals` (see `redirects()` in `next.config.ts`).

The Prisma models are deliberately left in place — see
`docs/aries-integration/STRIP-PLAN.md`, which separates the UI/API strip from
the schema cleanup so that a destructive migration never rides the same merge
as the code change.
...
FORUM_ADMIN_TOKEN         # Operator bearer token for administrative routes (>= 32 chars).
                          # Unset means those routes answer 503, not that they are open.
```
